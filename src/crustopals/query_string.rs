...
lazy_static! {
    pub static ref PREPEND_STR: String = "comment1=cooking%20MCs;userdata=".to_string();
    pub static ref APPEND_STR: String = ";comment2=%20like%20a%20pound%20of%20bacon".to_string();
}
...
pub fn cbc_decrypts_with_admin_rights(key: &[u8], iv: &[u8], ct_bytes: &[u8]) -> bool {
    let decrypted: Vec<u8> = cbc_decrypt(key, iv, ct_bytes);
    has_admin_rights(decrypted)
}

pub fn ctr_decrypts_with_admin_rights(key: &[u8], nonce: &[u8], ct_bytes: &[u8]) -> bool {
