...
pub fn decrypt_message_cbc(bytes: &[u8], key: &[u8], iv: &[u8]) -> Result<Vec<u8>, String> {
...
pub fn decrypt_message_ecb(bytes: &[u8], key: &[u8]) -> Result<Vec<u8>, String> {
...
mod tests {
    ...
    fn it_can_decrypt_messages_in_ctr_mode() {
        let key = "YELLOW SUBMARINE".as_bytes();
        let nonce = [0u8; 8];
        let ctr_encrypted_msg = base64::decode(
    ...
    fn decrypts_messages_in_cbc_mode() {
        let key = "YELLOW SUBMARINE".as_bytes();
        let iv = [0 as u8; 16];
        let ciphertext = base64::decode("nRGOqUe3iBURUPUe5NjYJTSrsoTAS1bzHKzpdPDcoFg=").unwrap();
    ...
    fn decrypts_messages_in_ecb_mode() {
        let key = "YELLOW SUBMARINE".as_bytes();
        let ciphertext = base64::decode("nRGOqUe3iBURUPUe5NjYJWD6NnB+RfSZ26DyW5IjAaU=").unwrap();
    ...
    fn decrypts_ecb_test_message() {
        ...
        let ciphertext =
            hex::decode("69c4e0d86a7b0430d8cdb78070b4c55a954f64f2e4e86e9eee82d20216684899")
                .unwrap();
