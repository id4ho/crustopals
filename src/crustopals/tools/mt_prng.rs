...
impl MT19937 {
    ...
    fn initialize(&mut self) {
        ...
        for i in 1..N {
            ...
            self.state[i] = F.wrapping_mul(prev_xor_prev_shifted).wrapping_add(i as u32);
        }
    }
    ...
    fn twist(&mut self) {
        ...
        for kk in 0..N {
            ...
            let y = (self.state[kk] & upper_mask) | (self.state[next_index] & lower_mask);
            if kk < (N - M) {
                self.state[kk] = self.state[kk + M] ^ (y >> 1) ^ (MATRIX_A * (y & 1));
            } else if kk < (N - 1) {
                self.state[kk] = self.state[kk - (N - M)] ^ (y >> 1) ^ (MATRIX_A * (y & 1));
            } else {
